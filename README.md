...
| 스크래치 | 쌍별 타수 차 × 타당 금액 직접 정산. **그 홀에 버디(이상) 또는 트리플보기(이상)가 1명이라도 있으면 해당 홀 정산 ×2**(속칭 '배판', par 기준 diff ≤ −1 또는 ≥ +3 감지, 결과에 "정산 2배" 표시). 버디·트리플 중복/복수 발생해도 **최대 ×2**(불리언 플래그, ×4 불가) |
...
- 첫 화면 좌측 상단 [사용방법](빨강) → **간단요약(진행 흐름 다이어그램, 첫 탭)** + 진행자/참여자/게임 룰/공통 규칙 가이드 모달

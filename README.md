...
| 후세인 | 직전 홀 2등 1명 vs 나머지. 후세인 점수는 상대 인원수만큼 곱해 공정 비교, 승리 시 설정금액×상대수 독식, 패배 시 상대 전원에게 설정금액 지급. 대결 방식 선택: 1·3·4등(4인 기준 본인×3 vs 3명 합) / 1·3등(본인×2 vs 1등+3등, 4등은 점수 제외·상금만 동일 참여). 2~3인은 상대수에 맞춰 ×1·×2로 자동 조정 |
...
- 단체전 → 개인전(스트로크): 팀 상금 전체(팀매치 ×2, 후세인 ×3 등)를 개인 승자에게 몰빵
...
- 게임선택 탭: 일반 게임 + Additional Option(신페리오) + 니어리스트·롱기스트(적용 홀·금액, 진행자가 홀에서 당첨자/PASS 선택, 선택 완료까지 홀 결과·다음홀 보류, 당첨금 은행→지갑)

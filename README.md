...
- 게임선택 탭: 일반 게임 + Additional Option(신페리오) + 니어리스트·롱기스트(적용 홀·금액, 진행자가 홀에서 당첨자/PASS 선택, 선택 완료까지 홀 결과·다음홀 보류, 당첨금 은행→지갑). 게임명 옆 (?)로 룰 팝업
  - **4인 미만이면 팀 게임(라스베가스·팀매치·좌탄우탄)은 선택 불가**(계산 로직은 유지, UI에서만 차단)
  - 후세인 선택 시 점수 비교 방식 카드(연합군 전원 / 최하위 1명 제외 — 4인 이상만)
  - 조폭 스킨스: 선택 시 18홀 전체 자동 점유(홀 선택 숨김) + 반납 강도 카드(더블50%·트리플100% / 파3만 한단계 엄격). 다른 메인 게임과 상호 배타(신페리오·니어·롱기는 병행 가능)

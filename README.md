...
│   │   ├── setup/[roomId]/        # 진행자 게임 설정 (게임선택→홀파→판돈→기타→시작)
...
| 팀 매치플레이 | 고정 팀(블루/오렌지), 팀 합산 타수 자동 판정, 무승부 이월. 스코어보드에 NUP/T 누적 표시 |
...
- 팀매치 시: 팀구성 이름을 팀 색(블루/오렌지)으로 표시, 전반·후반·전체홀 매치일 때 누적 UP 줄(1UP/2UP/T) 표시
...
- setup: 게임선택 → 홀파설정 → 판돈설정 → 기타 단계별 [다음] 버튼, 마지막에 [게임 시작]
- 기타 탭: OECD 설정 + 버디값 설정 (활성화 시 기본 10,000원, 같은 팀 버디값 받기 토글 — 기본 안받기)
